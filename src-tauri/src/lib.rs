...
use crate::queue_engine::QueueDiscipline;
use tauri::Window;
...
fn run_simulation(
    ...
    sim.run_simulate(num_of_prcss, queue, window, context_switch, time_quantum);
}

#[tauri::command]
fn run_with_parameters(
    window: Window,
    array_of_processes: Vec<(u64, u64)>,
    queue: &str,
    context_switch: u64,
    time_quantum: u64,
) {
    ...
    sim.run_with_predefined_processes(queue, window, context_switch, time_quantum, array_of_processes)
}
